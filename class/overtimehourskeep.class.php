...
/**
 * \file        class/overtimehourskeep.class.php
 * \ingroup     overtime
 * \brief       This file is a CRUD class file for OvertimeHoursKeep (Create/Read/Update/Delete)
 */
...
require_once 'overtime.class.php';

/**
 * Class for OvertimeHoursKeep
 */
class OvertimeHoursKeep extends CommonObject
{
	/**
	 * @var string ID of module.
	 */
	public $module = 'overtime';

	/**
	 * @var string ID to identify managed object.
	 */
	public $element = 'overtimehourskeep';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'overtime_overtimehourskeep';
	...
	/**
	 * @var string String with name of icon for overtimehourskeep. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'overtimehourskeep@overtime' if picto is file 'img/object_overtimehourskeep.png'.
	 */
	public $picto = 'fa-hourglass';
	...
	public $fields=array(
		...
		"fk_user" => array("type"=>"integer", "label"=>"User", "enabled"=>"1", 'position'=>40, 'notnull'=>1, "visible"=>"1",),
	);
	public $rowid;
	public $label;
	public $description;
	...
	//  */
	// public $table_element_line = 'overtime_overtimehourskeepline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_overtimehourskeep';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'OvertimeHoursKeepline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'OvertimeHoursKeep', 'fk_element'=>'fk_overtimehourskeep'));
	...
	//  */
	// protected $childtablesoncascade = array('overtime_overtimehourskeepdet');

	// /**
	//  * @var OvertimeHoursKeepLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('overtime', 'overtimehourskeep', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('overtime', 'overtimehourskeep', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('overtime', 'overtimehourskeep_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'overtimehourskeep/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'overtimehourskeep/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'overtimehourskeep/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'overtimehourskeep/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->overtime->dir_output.'/overtimehourskeep/'.$oldref;
				$dirdest = $conf->overtime->dir_output.'/overtimehourskeep/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->overtime->dir_output.'/overtimehourskeep/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowOvertimeHoursKeep")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("OvertimeHoursKeep").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/overtime/overtimehourskeep_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowOvertimeHoursKeep");
	...
	public function getLinesArray()
	{
		...
		$objectline = new OvertimeHoursKeepLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_overtimehourskeep = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (getDolGlobalString('OVERTIME_MYOBJECT_ADDON')) {
			...
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/overtime/");
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_overtimehourskeep';
		...
		$modelpath = "core/modules/overtime/doc/";
	...
	public function counted(User $user, Overtime $overtime)
	{
		global $conf, $langs;
		...
		require_once 'overtimedaycounted.class.php';

		$daycounted = new OvertimeDayCounted($this->db);
		...
		$hours_to_reserve = (int) $conf->global->OVERTIME_DAY_TO_RESERVE;

		if ($daycounted->dayreserve < $hours_to_reserve) {
			$hours_to_reserve_left = $hours_to_reserve - $daycounted->dayreserve;
			if ($this->hourskeeped > $hours_to_reserve_left) {
				$this->hourskeeped -= $hours_to_reserve_left;
				$daycounted->dayreserve += $hours_to_reserve_left;
			} else {
...
/**
 * Class OvertimeHoursKeepLine. You can also remove this and generate a CRUD class for lines objects.
 */
class OvertimeHoursKeepLine extends CommonObjectLine
{
	// To complete with content of an object OvertimeHoursKeepLine
	// We should have a field rowid, fk_overtimehourskeep and position
