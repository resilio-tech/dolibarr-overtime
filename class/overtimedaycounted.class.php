...
/**
 * \file        class/overtimedaycounted.class.php
 * \ingroup     overtime
 * \brief       This file is a CRUD class file for OvertimeDayCounted (Create/Read/Update/Delete)
 */
...
/**
 * Class for OvertimeDayCounted
 */
class OvertimeDayCounted extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'overtimedaycounted';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'overtime_overtimedaycounted';
	...
	//  */
	// public $table_element_line = 'overtime_overtimedaycountedline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_overtimedaycounted';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'OvertimeDayCountedline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'OvertimeDayCounted', 'fk_element'=>'fk_overtimedaycounted'));
	...
	//  */
	// protected $childtablesoncascade = array('overtime_overtimedaycounteddet');

	// /**
	//  * @var OvertimeDayCountedLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('overtime', 'overtimedaycounted', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'overtimedaycounted/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'overtimedaycounted/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'overtimedaycounted/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'overtimedaycounted/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->overtime->dir_output.'/overtimedaycounted/'.$oldref;
				$dirdest = $conf->overtime->dir_output.'/overtimedaycounted/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->overtime->dir_output.'/overtimedaycounted/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/overtime/overtimedaycounted_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowOvertimeDayCounted");
	...
	public function getLinesArray()
	{
		...
		$objectline = new OvertimeDayCountedLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_overtimedaycounted = '.((int) $this->id)));
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_overtimedaycounted';
...
/**
 * Class OvertimeDayCountedLine. You can also remove this and generate a CRUD class for lines objects.
 */
class OvertimeDayCountedLine extends CommonObjectLine
{
	// To complete with content of an object OvertimeDayCountedLine
	// We should have a field rowid, fk_overtimedaycounted and position
