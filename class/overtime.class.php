...
/**
 * \file        class/overtime.class.php
 * \ingroup     overtime
 * \brief       This file is a CRUD class file for Overtime (Create/Read/Update/Delete)
 */
 ...
 * Class for Overtime
 */
class Overtime extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'overtime';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'overtime_overtime';
	...
	const STATUS_DECOMPTED = 4;
	const STATUS_REMBOURSED = 7;
	...
	public $fields=array(
		...
		'status' => array('type'=>'integer', 'label'=>'Status', 'enabled'=>'1', 'position'=>2000, 'notnull'=>1, 'visible'=>1, 'index'=>1, 'arrayofkeyval'=>array('0'=>'Brouillon', '1'=>'Validé', '2'=>'Décompté', '3'=>'Remboursé', '9'=>'Annulé'), 'validate'=>'1',),
	...
	//  */
	// public $table_element_line = 'overtime_overtimeline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_overtime';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Overtimeline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Overtime', 'fk_element'=>'fk_overtime'));
	...
	//  */
	// protected $childtablesoncascade = array('overtime_overtimedet');

	// /**
	//  * @var OvertimeLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('overtime', 'overtime', 'read')) {
	...
	public function create(User $user, $notrigger = false)
	{
		...
		$sql .= " '".$this->db->escape($this->reason)."'";
		$sql .= ")";
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('overtime','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && !empty($user->rights->overtime->overtime->overtime_advance->validate))))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'overtime/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'overtime/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++; $this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'overtime/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'overtime/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++; $this->error = $this->db->lasterror();
				}
				...
				$dirsource = $conf->overtime->dir_output.'/overtime/'.$oldref;
				$dirdest = $conf->overtime->dir_output.'/overtime/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->overtime->dir_output.'/overtime/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowOvertime")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Overtime").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/overtime/overtime_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowOvertime");
	...
	public function getLinesArray()
	{
		...
		$objectline = new OvertimeLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_overtime = '.((int) $this->id)));
	...
	private function setOvertimeStatus($status) {
		$this->statut = $status;

		$sql = "UPDATE ".MAIN_DB_PREFIX.$this->table_element;
		$sql .= " SET status = ".((int) $this->statut);
		$sql .= " WHERE rowid = ".((int) $this->id);
	...
	public function setOvertimeCounted() {
		return $this->setOvertimeStatus(self::STATUS_DECOMPTED);
	}

	public function setOvertimeRefunded() {
...
/**
 * Class OvertimeLine. You can also remove this and generate a CRUD class for lines objects.
 */
class OvertimeLine extends CommonObjectLine
{
	// To complete with content of an object OvertimeLine
	// We should have a field rowid, fk_overtime and position
