...
/**
 *   	\file       overtimedaycounted_list.php
 *		\ingroup    overtime
 *		\brief      List page for overtimedaycounted
 */
...
require_once __DIR__.'/class/overtimedaycounted.class.php';
...
if (!isModEnabled("overtime")) {
	accessforbidden('Module overtime not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'OvertimeDayCounted';
	$objectlabel = 'OvertimeDayCounted';
...
$title = $langs->trans("OvertimeDayCounteds");
//$help_url = "EN:Module_OvertimeDayCounted|FR:Module_OvertimeDayCounted_FR|ES:Módulo_OvertimeDayCounted";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/overtime/overtimedaycounted_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-overtime page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$topicmail = "SendOvertimeDayCountedRef";
$modelmail = "overtimedaycounted";
$objecttmp = new OvertimeDayCounted($db);
...
if ($search_all) {
	...
	print '<!-- Search done like if OVERTIMEDAYCOUNTED_QUICKSEARCH_ON_FIELDS = '.$setupstring.' -->'."\n";
